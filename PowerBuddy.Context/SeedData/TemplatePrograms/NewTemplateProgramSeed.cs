...
namespace PowerBuddy.Data.Context.SeedData.TemplatePrograms
{
    public static class NewTemplateProgramSeed
    {
        ...
        {
            var squat = exercises.Where(x => x.ExerciseName.Contains("(B) Back Squat")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var bench = exercises.Where(x => x.ExerciseName.Contains("(B) Bench Press")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var declineBench = exercises.Where(x => x.ExerciseName.Contains("(B) Decline Bench Press")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var deadlift = exercises.Where(x => x.ExerciseName.Contains("(B) DeadLift")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var deadliftToKnee = exercises.Where(x => x.ExerciseName.Contains("(B) Deadlift (to knee")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var deadliftFromBoxes = exercises.Where(x => x.ExerciseName.Contains("(B) Deadlift (from boxes)")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var deficitDeadlift = exercises.Where(x => x.ExerciseName.Contains("(B) Deadlift (deficit)")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var benchCloseGrip = exercises.Where(x => x.ExerciseName.Contains("(B) Bench Press (Close Grip)")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            var deadliftPaused = exercises.Where(x => x.ExerciseName.Contains("(B) Deadlift (Paused)")).Select<Exercise, int>(x => x.ExerciseId).FirstOrDefault();
            ...
            return new TemplateProgram
            {
                ...
                TemplateWeeks = new List<TemplateWeek>
                {
                    new TemplateWeek
                    {
                        WeekNo = 1, TemplateDays = new List<TemplateDay>
                        {
                            new TemplateDay
                            {
                    ...
                    new TemplateWeek
                    {
                        WeekNo = 2, TemplateDays = new List<TemplateDay>
                        {
                            new TemplateDay
                            {
                                DayNo = 1, TemplateExercises = new List<TemplateExercise>
                    ...
                    new TemplateWeek
                    {
                        WeekNo = 3, TemplateDays = new List<TemplateDay>
                        {
                            new TemplateDay
                            {
                                DayNo = 1, TemplateExercises = new List<TemplateExercise>
                            ...
                            new TemplateDay
                            {
                                DayNo = 2, TemplateExercises = new List<TemplateExercise>
                                {
                                    ...
                                    new TemplateExercise
                                    {
                                        ExerciseId = bench,
                                        RepSchemeType = Enum.GetName(typeof(RepSchemeTypeEnum), RepSchemeTypeEnum.Ramped),
                                        NoOfSets = 8, RepSchemeFormat = "1x5, 1x4, 1x3, 2x3, 3x2", TemplateRepSchemes = new List<TemplateRepScheme>
                                        {
                                            new TemplateRepScheme {SetNo = 1, NoOfReps = 5, Percentage = 50},
                                            new TemplateRepScheme {SetNo = 2, NoOfReps = 4, Percentage = 60},
                                            new TemplateRepScheme {SetNo = 3, NoOfReps = 3, Percentage = 70},
                    ...
                    new TemplateWeek
                    {
                        WeekNo = 4, TemplateDays = new List<TemplateDay>
                        {
                            new TemplateDay
                            {
                                DayNo = 1, TemplateExercises = new List<TemplateExercise>
                                {
                                    new TemplateExercise
                                    {
                                        ExerciseId = squat, RepSchemeType = Enum.GetName(typeof(RepSchemeTypeEnum), RepSchemeTypeEnum.Ramped),
